...
if len(lab_ids) == 0:
    raise RuntimeError("No itemids found for WBC/LACTATE/CREATININE. Check d_labitems labels.")
...
agg = (labs
       .groupby(["subject_id","hadm_id","admission_type",
                 "anchor_age","anchor_year_group","gender","label"], as_index=False)
       .agg(value=("valuenum","median")))

wide = (agg
        .pivot_table(index=["subject_id","hadm_id","admission_type",
                            "anchor_age","anchor_year_group","gender"],
...
wide = wide.merge(hadm_times, on="hadm_id", how="left").rename(columns={"admittime":"charttime"})

# ---- 7) Canonical schema + sort ----
COLS = [
    "subject_id","hadm_id",
    "anchor_age","gender","admission_type",
    "wbc","lactate","creatinine",
    "charttime","anchor_year_group",
]
