...
def audit_positive_rate(
    df: pd.DataFrame, group: str, label: str = "label"
) -> pd.DataFrame:
    rows = []
...
def audit_fairness(*args, **kwargs):
    ...
    # Case 1: DataFrame provided
        ...
        raise TypeError(
            "audit_fairness() expects either (df, group, label=...) or (data_csv, group, out_csv, out_md, label=...)."
        )
...
def main() -> int:
    ...
    try:
        audit_fairness(
            args.data, args.group, args.out_csv, args.out_md, label=args.label
        )
        print(f"✅ fairness metrics → {args.out_csv}")
        print(f"✅ fairness report  → {args.out_md}")
        return 0
    except Exception as e:
        pd.DataFrame(columns=["metric", "group", "value"]).to_csv(
