...
from src.eval.performance_metrics import compute_from_predictions_csv as _perf_from_preds  # adjust if different
...
async def validate(
    ...
    if {"y_true", "y_score"}.issubset(set(df.columns)):
        ...
        perf = json.loads(Path("reports/performance_metrics.json").read_text()) if _perf_from_preds(str(tmp_path)) is None else {}
    ...
    try:
        _audit_fairness("data/data_prepared_current.csv", None, fair_csv, fair_md)  # use current prepared data
    except Exception:
        pass
    ...
    gate = {}
    try:
        gate = json.loads(Path("reports/policy_gate_result.json").read_text(encoding="utf-8"))
    except Exception:
        pass
    ...
    Path("reports/live_validation.json").write_text(json.dumps(result, indent=2), encoding="utf-8")
    return result
...
if __name__ == "__main__":
    ...
    import uvicorn
    uvicorn.run("src.api.validation_server:app", host="0.0.0.0", port=int(os.getenv("PORT", "7860")), reload=False)